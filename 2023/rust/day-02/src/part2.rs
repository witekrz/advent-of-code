...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let x: u32 = output.iter().map(|game| game.multiple()).sum();
...
impl Game {
    ...
    pub fn multiple(&self) -> u32 {
        let z: Vec<PairColorValue> = self.cube_sets.clone().into_iter().map(|z| z.color_value).flatten().collect();
        let max_red = z.iter().filter(|a| a.color == "red").map(|b| b.value).max().unwrap();
        let max_green = z.iter().filter(|a| a.color == "green").map(|b| b.value).max().unwrap();
        let max_blue = z.iter().filter(|a| a.color == "blue").map(|b| b.value).max().unwrap();
